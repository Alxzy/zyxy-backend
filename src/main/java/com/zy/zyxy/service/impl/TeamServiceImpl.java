...
import static com.zy.zyxy.contant.TeamConstant.USER_JOIN_TEAM_TEAM_KEY;
...
//            team.setStatus(TeamStatusEnum.PUBLIC.getValue());
    ...
    public boolean joinTeam(Long teamId, User loginUser, String password) {
        ...
        String userKey = String.format(USER_JOIN_TEAM_USER_KEY, userId);
        RLock userLock = redissonClient.getLock(userKey);
        try {
            // 抢分布式锁
            // todo 两把分布式锁? 队伍锁 和 用户锁?  选择用户锁 队伍人数超了就超了
            // 引入一个变量 获取锁多次则失效
            int getCount = 0;
            while (true) {
                // 只有一个线程能获取到锁
                // 注意这里尽量不要设置获取锁的超时时间,不然容易发生死锁!
                getCount ++;
...
//        isExistTeam(teamId);
    ...
    @Override
    public List<TeamUserVO> getResultTeamVOList(List<TeamUserVO> resultList,HttpServletRequest request) {
        // 1.判断当前用户是否已加入队伍
        ...
        resultList.forEach(team -> {
            ...
            team.setCreateAvatarUrl(team.getCreateUserVO().getAvatarUrl());
            team.setCreateUsername(team.getCreateUserVO().getUsername());
        });
        return resultList;
    }
